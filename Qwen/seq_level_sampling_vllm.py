...
class LLM_Word_Level_Ensemble:
    ...
    def __init__(self, model_name ):
        ...
        self.model =  LLM(
                model=model_name,
                tensor_parallel_size=4,  # use all 3 GPUs, adjust as needed
                dtype="bfloat16",        # or "auto", "float16" etc.
            )
    ...
    def pipeline_process(self, batch_size , log_file , num_beam=1 , top_k=1 , max_num_token=400 , start_index=0 , end_index=-1 , thread_number=0):
        ...
        for index in range( number_of_batches ):
            ...
            #     )
            params = BeamSearchParams(beam_width=num_beam, max_tokens=50, logprobs=5)
            
            outputs = self.model.beam_search(batch, params)

            inputs_log_prob = []
            most_probable_gen_text = []
            for output in outputs:
                for sequence in output.sequences:
                    generated_text = sequence.text
                    most_probable_gen_text.append(generated_text)
                    log_probs = sequence.logprobs
                    inputs_log_prob.append(log_probs)
                    print(f"Generated text: {generated_text}, Log probabilities: {log_probs}")
