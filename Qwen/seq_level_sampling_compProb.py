...
class LLM_Word_Level_Ensemble:
    ...
    def codeS_prompt_preprocessing(self, prompt, demo_stealing=None):
        ...
        if demo_stealing is None:
            demo_stealing_parts = prompt_parts#[0:5] # No stealing
        else:
            demo_stealing_parts = split_text_by_schema(demo_stealing)
        # combined_text = 'Please pay attention to the following demonstration. A similar question will be given to you after.'
        # for prompt_part_id in range(len(demo_stealing_parts)):
        #     combined_text = combined_text +  f'\nDemonstration number {prompt_part_id+1}:\n' + demo_stealing_parts[prompt_part_id] + '\n' + eos +'\n'
        # combined_text+=  '\nQuestion:'
        combined_text = eos + prompt_parts[-1]
    ...
    def start_pipelines(self, batch_size , log_file , num_beam=1 , top_k=1 , max_num_token=200 ,  num_threads = 3):
        ...
        for i , my_thread in enumerate(thread_list):
            thread_output_list, thread_inputs_log_prob_list, thread_context_log_prob_list, thread_question_log_prob_list, thread_batch_text_list = my_thread.join()
...
if __name__ == "__main__":
    ...
    list_of_outputs, list_of_inputs_log_prob, list_of_context_log_prob, list_of_question_log_prob, list_of_batch_text= word_ensemble.start_pipelines( batch_size , opt.log_file, num_beam=opt.num_beam , top_k=opt.num_beam , num_threads = opt.thread_num )
