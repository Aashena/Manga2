...
class LLM_Word_Level_Ensemble:
    ...
    def __init__(self, model_name ):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_name , torch_dtype = torch.bfloat16,  #attn_implementation="flash_attention_2",
                                                            cache_dir='/mnt/nvme/yadegari/hub', device_map="auto" )#.to(self.device_list[0])#{"": accelerator.process_index}) ###NO DEVICE
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_name , cache_dir='/mnt/nvme/yadegari/hub')
        self.model.eval()
    ...
    def codeS_prompt_preprocessing(self, prompt, demo_stealing=None):
        ...
        combined_text = 'Congrats! You have successfully answered the following questions. You are very knowledgeable about Databases and SQL. Each answer ends with a semicolon (;).'
        for prompt_part_id in range(len(demo_stealing_parts)):
            combined_text += f'\n{demo_stealing_parts[prompt_part_id]}\n'
        combined_text += prompt_parts[-1] #eos + prompt_parts[-1]
    ...
    def pipeline_process(self, batch_size , log_file , num_beam=1 , top_k=1 , max_num_token=400 , start_index=0 , end_index=-1 , thread_number=0):
        ...
        for index in range( number_of_batches ):
            ...
            for propt_idx in range(len(batch)):
                ...
                batch[propt_idx] = batch[propt_idx] + '\nSELECT' #This is for bird datasets except when using OneSQL or CodeS!
                # This is for OneSQL:
            ...
            candid_set = set()
            counter = 0
            dynamic_temprature = 0.8
            while len(candid_set)<num_beam:
                counter+=1
                print(f'number of iteration: {counter}, len(candid_set): {len(candid_set)}')
                with torch.no_grad():
                    output = self.model.generate(
                        **inputs,
                        max_new_tokens=200,
                        do_sample = True,
                        num_return_sequences=16,  # Return all beams
                        temperature=dynamic_temprature,
                        #length_penalty=0.5,
                        use_cache = True,
                ...
                # print(output.keys())
                generated_tokens = output[ : , inputs_len: ]
                ...
                old_num_candids = len(candid_set)
                candid_set.update(most_probable_gen_text)
                new_num_candids = len(candid_set)
                if old_num_candids==new_num_candids:
                    dynamic_temprature = dynamic_temprature + 0.05
                del output

            candid_list = list(candid_set)[:num_beam]

            for sql in candid_list:
                print(f'generated_sqls: ' , sql)
            print('------------------------')
            ...
            with open(log_file , 'a' ) as f:
                ...
                f.write(f'\nThe chosen gen text:  {candid_list}\n')
            list_of_outputs.extend( candid_list )
        print(f'Thread Num: {thread_number}')
        for i in self.device_list:
            print(f'End of process | max {i}:  , {torch.cuda.max_memory_allocated(i)} ')
        # print('\n')

        return list_of_outputs #, list_of_batch_text
...
if __name__ == "__main__":
    ...
    # dataset1 = 'codes-1b_BIRD_table_num_6_column_num_10_5-shot_5-10_wEvidence_max_tokens_8192_max_new_tokens_256.json'
    ...
    # dataset1 = 'OmniSQL_BIRD_0-shot_wEvidence.json'
