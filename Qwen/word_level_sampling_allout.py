...
class LLM_Word_Level_Ensemble:
    ...
    def __init__(self, model_name ):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_name , torch_dtype = torch.float16,
                                                           attn_implementation="flash_attention_2" ,trust_remote_code=True).to(self.device_list[0])#{"": accelerator.process_index}) ###NO DEVICE
    ...
    def get_predictions(self, inputs , past_key_values_tensor=None):
        ...
        with torch.no_grad():
            ...
            if past_key_values_tensor is not None:
                inputs = inputs[: , -1:]
                pkv = self.convert_pkvTensor_into_pkv(past_key_values_tensor)
            ...
            return model_output['logits'][:,-1,:], self.convert_pkv_into_tensor( model_output['past_key_values'] )
    ...
    def get_next_word_score(self, sentences, past_key_values_tensor=None ,top_k=5):#operation_on_a_batch
        ...
        if active_texts_index.size(0)>0:
            ...
            past_key_values_tensor = torch.empty( past_key_values_tensor_size ,dtype=torch.float16 ).to(self.device_list[0])
...
if __name__ == "__main__":
    ...
    dataset_list = [dataset5 ]#, dataset2 , dataset3 , dataset4 , dataset5]
