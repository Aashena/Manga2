    ...
    def get_predictions(self, inputs , past_key_values_tensor=None , num_comp=5 , init_att_pkv_tuple = None,
        ...
        with torch.no_grad():
            if past_key_values_tensor is not None:
                ...
                # print("model_output['past_key_values'][0].size(): " , model_output['past_key_values'][0].size())
                return model_output['logits'][:,-1,:], torch.stack( model_output['past_key_values'] ) , None
                    ...
                    for index in range( new_inputs.size(-1) ):
                        ...
                        if index == 0:
                            ...
                            model_output = self.model( step_input , attention_mask=new_attention_mask,
                                use_cache=True, return_dict=True, past_key_values=torch.unbind(init_pkv_tensor,dim=0) )
                ...
                for beam , past_key_values_tensor in enumerate(past_key_values_tensor_list):
                    ...
                    padding_tensor = padding_tensor.expand(-1,-1, number_of_paddings , -1)
                ...
                past_key_values_tensor = torch.stack( past_key_values_tensor_list , dim=2).reshape(past_key_values_tensor_list[0].size(0), -1, past_key_values_tensor_list[0].size(2), past_key_values_tensor_list[0].size(3))
                padding_tensor = past_key_values_tensor[:,:,0:1,:]
    ...
    def get_next_word_score(self, sentences, just_performed_ensemble, current_clause_length,
        ...
        if active_texts_index.size(0)>0:
            ...
            if past_key_values_tensor == None:
                ...
                past_key_values_tensor = torch.zeros( past_key_values_tensor_size ,dtype=torch.float16 ).to(self.device_list[0]) # past_key_values_tensor = torch.cat( [ past_key_values_tensor[:,:,0:1,:] , past_key_values_tensor ] , dim=2 ) #
...
if __name__ == "__main__":
    ...
    # dataset1 = 'codes-1b_BIRD_table_num_5_column_num_6_5-shot_shuffled1_max_tokens_8192_max_new_tokens_256.json'
    # dataset2 = 'codes-1b_BIRD_table_num_5_column_num_6_5-shot_shuffled2_max_tokens_8192_max_new_tokens_256.json'
    # dataset3 = 'codes-1b_BIRD_table_num_5_column_num_6_5-shot_shuffled3_max_tokens_8192_max_new_tokens_256.json'
    # dataset4 = 'codes-1b_BIRD_table_num_5_column_num_6_5-shot_shuffled4_max_tokens_8192_max_new_tokens_256.json'
    # dataset5 = 'codes-1b_BIRD_table_num_5_column_num_6_5-shot_shuffled5_max_tokens_8192_max_new_tokens_256.json'
